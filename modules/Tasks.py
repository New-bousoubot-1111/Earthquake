...
sindoMap = {
  -1: {'title': '不明', 'color': 0x000000},
  10: {'title': '震度1', 'color': 0xf2f2ff},
  20: {'title': '震度2', 'color': 0x00aaff},
  30: {'title': '震度3', 'color': 0x0040ff},
  40: {'title': '震度4', 'color': 0xfae696},
  50: {'title': '震度5弱', 'color': 0xffe600},
  55: {'title': '震度5強', 'color': 0xff9900},
  60: {'title': '震度6弱', 'color': 0xff2600},
  65: {'title': '震度6強', 'color': 0xa50021},
  70: {'title': '震度7', 'color': 0xb40069}
}
        ...
        try:
            ...
            async for websocket in client.connect('wss://api.p2pquake.net/v2/ws'):
                ...
                try:
                    while True:
                        ...
                        if data['code'] == 551:
                            ...
                            embed = nextcord.Embed(
                                title='地震情報',
                                description=f'''<t:{int(time)}:F>頃地震がありました。
                                震源地は、{hypocenter["name"]}{longitude_and_latitude}で、最大震度は{sindo_data["title"]}、震源の深さは{depth}、マグニチュードは{hypocenter["magnitude"]}と推定されます。
                                ''',
                    ...
                    log.error(f"WebSocket接続が切断されました: {e}")
                    continue  # 再接続を試みる

        except Exception as e:
            log.error(f"WebSocket接続に失敗しました: {e}")
